...
\begin{document}
  ...
  \begin{frame}{Verbes}
    ...
    \begin{columns}
      \column{0.4\textwidth}
        ...
        \begin{center}
          \includegraphics[scale=0.18]{dragon.jpg}
        \end{center}
      \column{0.6\textwidth}
        <<Le matin, moi et mon petit oiseau \underline{\uncover<2->{nous réveillons}} (se réveiller).
        Je \underline{\uncover<3->{descends}} (descendre) de mon lit jaune.
        Je \underline{\uncover<4->{me brosse}} (se brosser) les cheveux blancs.
        Mon ami, Atreyu, \underline{\uncover<5->{préfère}} (préférer) rester longtemps dans le lit, parce que lui et mon oiseau \underline{\uncover<6->{sortent}} (sortir) tard la nuit.>>
        Tu \underline{\uncover<7->{ne t'intéresses pas}} (ne pas s'intéresser) à cette histoire, alors tu vas au \hyperlink{début}{début}...
    \end{columns}
  \end{frame}
  ...
  \begin{frame}{Adverbes}
    ...
    \begin{columns}
      \column{0.4\textwidth}
        \begin{center}
          \includegraphics[scale=0.075]{araignée.jpg}
        \end{center}
        Tu te trouves devant une araignée géante!
        Elle te dit que tu peux passer si tu réponds bien à ces questions.
      \column{0.6\textwidth}
      \begin{enumerate}
        \item Je mange \underline{\uncover<2->{assez d'}} (enough) insectes.
        \item Je vais à la bibliothèque \underline{\uncover<3->{toutes les semaines}} (every week) pour prendre un livre.
        \item Tu penses que je \underline{\uncover<4->{ne me coiffe jamais}} (never does her hair), mais ce n'est pas vrai!
        \item Je me dépêche \underline{\uncover<5->{trop}} (too much)!
        \item Alor, je fais \underline{\uncover<6->{quelquefois}} (sometimes) des erreurs.
      \end{enumerate}
      \uncover<7->{
        Elle ment: tu ne peux pas passer.
        Elle te mange.
        Tu est \hyperlink{début}{mort}.
      }
    \end{columns}
  \end{frame}
